...
class Profile extends KComponent {
  constructor(props) {
    super(props);
    const userState = userInfo.getState() || {}; // Provide an empty object as default if null
    this.state = {
      data: userState,
      loading: false,
      error: "",
      optionStatus: userState.statusId || "", // Provide default value
      optionCountry: userState.country || "", // Provide default value
      optionGender: userState.gender || "" // Provide default value
    };
    ...
    const birthday = {
      year: bd ? bd.getFullYear() : null,
      date: bd ? bd.getDate() : null,
      month: bd ? bd.getMonth() : null
    };
